...
| Checkout | `checkout/index.blade.php`, `confirmation.blade.php`, `CheckoutController.php`, `FrontendCheckoutService.php` | Mostly dynamic | **Checkout requires a customer account** (routes gated by `auth`; guests are redirected to login) — every order is tied to a user. Shipping methods, payment methods, and tax are admin-backed; totals recompute live (free-over + tax). **Placing an order creates a real `Order` + `OrderDetails` (re-priced server-side), decrements stock, and the confirmation shows the real order.** The shipping form **prefills from the customer's default saved address**. **Coupon codes are real** — validated against the `Coupon` model (expiry, min-spend, usage cap) via an AJAX check, discount applied + re-validated server-side in `placeOrder`, and `used_count` incremented. **Online methods (ABA / Wallet) go through the ABA PayWay gateway** (`PaywayService`) — redirect to hosted checkout, server-to-server callback confirms via `check-transaction-2`, order auto-marked **paid**; manual stays confirm-by-admin. Remaining: per-step field validation. |
...
- **ABA PayWay** gateway integrated for online methods (see `docs/payway-integration.md`): signed Purchase form → hosted checkout → CSRF-exempt pushback + customer return both confirm via `check-transaction-2` and mark the order paid (idempotent). Needs sandbox credentials in `.env` + a verify of the hash field order/endpoints against your PayWay guide.
- **Remaining:** per-step field validation, and robuster variant matching (currently matches cart size/colour labels against variant attribute values; falls back to the product base price when no variant matches). The shipping form now **prefills from the customer's default saved address**.
