...
<div class="ut-card" style="padding:26px;max-width:540px;margin-bottom:18px">
    <form class="ut-col" style="gap:16px" method="POST" action="{{ route('frontend.account.password.update') }}">
        @csrf
        @method('PUT')
        <div class="field"><label>Current password</label><div style="position:relative"><input class="ut-input" type="password" name="current_password" placeholder="Current password" style="padding-right:64px" required><button type="button" data-toggle-pw style="position:absolute;right:12px;top:11px;border:0;background:none;color:var(--text-2);font-family:var(--font-head);font-weight:600;font-size:12.5px">Show</button></div>
            @error('current_password')<span style="color:var(--accent);font-size:12.5px;margin-top:6px;display:block">{{ $message }}</span>@enderror
        </div>
        <div class="field"><label>New password</label><div style="position:relative"><input class="ut-input" type="password" name="password" placeholder="New password" style="padding-right:64px" required><button type="button" data-toggle-pw style="position:absolute;right:12px;top:11px;border:0;background:none;color:var(--text-2);font-family:var(--font-head);font-weight:600;font-size:12.5px">Show</button></div>
            @error('password')<span style="color:var(--accent);font-size:12.5px;margin-top:6px;display:block">{{ $message }}</span>@enderror
        </div>
        <div class="field"><label>Confirm new password</label><div style="position:relative"><input class="ut-input" type="password" name="password_confirmation" placeholder="Re-enter new password" style="padding-right:64px" required><button type="button" data-toggle-pw style="position:absolute;right:12px;top:11px;border:0;background:none;color:var(--text-2);font-family:var(--font-head);font-weight:600;font-size:12.5px">Show</button></div></div>
