...
@section('title', 'My Wallet — T-Shirt Shop')

@section('account')
<div class="ut-row" style="justify-content:space-between;align-items:flex-end;margin-bottom:18px;gap:12px;flex-wrap:wrap">
    <div><h2 style="font-size:24px">My wallet</h2><p class="muted" style="font-size:14px;margin-top:4px">Store credit you can spend at checkout</p></div>
</div>
...
<div class="ut-card" style="padding:26px;margin-bottom:20px">
    <div class="ut-row" style="justify-content:space-between;gap:20px;flex-wrap:wrap;align-items:flex-end">
        <div>
            <div class="muted" style="font-size:13px">Available balance</div>
            <div style="font-family:var(--font-head);font-weight:800;font-size:38px;line-height:1.1">${{ number_format($balance, 2) }}</div>
        </div>
        <form method="POST" action="{{ route('frontend.account.wallet.topup') }}" class="ut-row" style="gap:10px;align-items:flex-end;flex-wrap:wrap">
            @csrf
            <div class="field"><label>Top up amount ($)</label><input class="ut-input" type="number" name="amount" min="1" step="0.01" value="{{ old('amount') }}" placeholder="20.00" required style="width:160px"></div>
            <button class="ut-btn ut-btn-ink ut-btn-lg" type="submit">Top up via ABA PayWay</button>
        </form>
    </div>
...
{{-- transaction history --}}
<h3 style="font-size:17px;margin-bottom:12px">Activity</h3>
    ...
    @forelse($transactions as $t)
        <div class="ut-row" style="justify-content:space-between;gap:14px;padding:14px 16px;{{ !$loop->last ? 'border-bottom:1px solid var(--border-2)' : '' }}">
            ...
            <div style="text-align:right">
                <div style="font-family:var(--font-head);font-weight:700;color:{{ (float) $t->amount < 0 ? 'var(--accent)' : '#15803d' }}">{{ (float) $t->amount < 0 ? '−' : '+' }}${{ number_format(abs((float) $t->amount), 2) }}</div>
                <div class="muted" style="font-size:12px">Balance ${{ number_format((float) $t->balance_after, 2) }}</div>
            </div>
        </div>
    @empty
        <div style="padding:48px;text-align:center">
            ...
            <h3>No wallet activity yet</h3><p class="muted" style="margin-top:6px">Top up your wallet to pay faster at checkout.</p>
        </div>
    @endforelse
</div>
@endsection
