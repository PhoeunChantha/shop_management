...
<div class="ut-card" style="padding:26px">
    ...
    <form class="ut-col" style="gap:16px;max-width:540px" method="POST" action="{{ route('frontend.account.profile.update') }}">
        ...
        <div class="ut-form-2" style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div class="field"><label>First name</label><input class="ut-input" name="first_name" value="{{ old('first_name', $user['first']) }}" required>
                @error('first_name')<span style="color:var(--accent);font-size:12.5px;margin-top:6px;display:block">{{ $message }}</span>@enderror
            </div>
            <div class="field"><label>Last name</label><input class="ut-input" name="last_name" value="{{ old('last_name', $user['last']) }}"></div>
        </div>
        <div class="field"><label>Email address</label><input class="ut-input" type="email" value="{{ $user['email'] }}" disabled><small class="muted" style="font-size:12px;margin-top:6px;display:block">Email can’t be changed here.</small></div>
        <div class="field"><label>Phone number</label><input class="ut-input" name="phone" value="{{ old('phone', $user['phone']) }}" placeholder="+855 12 345 678">
            @error('phone')<span style="color:var(--accent);font-size:12.5px;margin-top:6px;display:block">{{ $message }}</span>@enderror
        </div>
        <div class="ut-row" style="gap:10px;margin-top:4px"><button class="ut-btn ut-btn-ink" type="submit">Save changes</button><a class="ut-btn ut-btn-ghost" href="{{ route('frontend.account.dashboard') }}">Cancel</a></div>
    </form>
</div>
@endsection
