...
@php
    $steps = [
        [__('Order placed'), __('We received your order').' #UT-'.$order['id'], $order['date'], 'checkC'],
        [__('Confirmed'), __('Payment confirmed and order accepted'), $order['date'], 'check'],
        [__('Processing'), __('Your items are being picked & packed'), 'Jun 4, 2026', 'box'],
        [__('Shipped'), __('Handed to').' '.$order['courier'].' · '.$order['tracking'], 'Jun 5, 2026', 'truck'],
        [__('Delivered'), $order['status'] === 'Delivered' ? __('Left at front door') : __('Estimated').' '.$order['eta'], '', 'home'],
    ];
@endphp
<div class="ut-wrap anim-up" style="padding-top:28px;max-width:760px">
    ...
    <div class="ut-card" style="padding:clamp(22px,4vw,32px);margin-bottom:20px">
        <div class="ut-row" style="justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:8px">
            <div><span class="ut-eyebrow" style="color:var(--blue)">{{ __('Order') }} #UT-{{ $order['id'] }}</span><h1 style="font-size:clamp(26px,3vw,34px);margin-top:8px">{{ $order['status'] === 'Delivered' ? __('Delivered') : __('On its way') }}</h1></div>
            ...
            @foreach($steps as $i => [$label, $desc, $date, $ic])
                ...
                <div class="ut-row" style="gap:18px;align-items:flex-start;{{ !$loop->last ? 'padding-bottom:28px' : '' }};position:relative">
                    ...
                    <div style="padding-top:3px">
                        <div class="ut-row" style="gap:10px;flex-wrap:wrap"><span style="font-family:var(--font-head);font-weight:700;font-size:15.5px;color:{{ $done ? 'var(--ink)' : 'var(--text-2)' }}">{{ $label }}</span>@if($current)<span class="ut-tag ut-tag-new" style="padding:2px 9px">{{ __('Current') }}</span>@endif</div>
