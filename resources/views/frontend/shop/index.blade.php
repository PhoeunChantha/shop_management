...
@section('title', 'Shop All Tees — T-Shirt Shop')
...
<div class="anim-up">
    ...
    <div style="background:#fff;border-bottom:1px solid var(--border)">
        <div class="ut-wrap" style="padding:30px 24px 24px">
            <x-frontend.breadcrumb :items="[['Home', route('frontend.home')], ['Shop all tees', null]]" />
            <div class="ut-row" style="justify-content:space-between;flex-wrap:wrap;gap:16px;align-items:flex-end;margin-top:8px">
                <div><h1 style="font-size:clamp(30px,4vw,46px)">All Tees</h1><p class="muted" style="margin-top:6px">{{ count($products) }} products · heavyweight organic cotton</p></div>
                <div style="position:relative;min-width:280px">
                    ...
                    <input class="ut-input" id="shopSearch" placeholder="Search tees…" style="padding-left:42px;border-radius:var(--r-pill)" oninput="filterProducts()">
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="ut-col" style="gap:26px">
                    ...
                    <div>
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:8px">
                            @foreach($sizes as $s)
                                <button type="button" class="ut-chip" style="width:50px;justify-content:center;padding:9px 0" onclick="this.classList.toggle('is-active')">{{ $s }}</button>
                            @endforeach
                        </div>
                    </div>
                    <hr class="divider">
                    <div>
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:12px">
                            @foreach($colors as $k => $c)
                                <button type="button" style="border:0;background:none;padding:0" title="{{ $c['name'] }}" onclick="this.firstElementChild.classList.toggle('is-active')">
                    ...
                    <hr class="divider">
                    <div>
                        <div style="font-family:var(--font-head);font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px">Max price — <span id="priceVal" style="color:var(--blue)">$120</span></div>
                        <input type="range" min="38" max="120" value="120" style="width:100%;accent-color:var(--blue)" oninput="document.getElementById('priceVal').textContent='$'+this.value; window.UT_MAXPRICE=+this.value; filterProducts()">
                        <div class="ut-row muted" style="justify-content:space-between;font-size:12px;margin-top:4px"><span>$38</span><span>$120</span></div>
                    </div>
                </div>
            </aside>
            ...
            <div>
                ...
                <div class="ut-results-grid" id="productGrid">
                    @foreach($products as $p)
                        <div class="product-cell" data-cat="{{ $p['cat'] }}" data-subcat="{{ $p['subcat'] }}" data-brand="{{ $p['brand'] }}" data-sale="{{ $p['tag'] === 'sale' ? 1 : 0 }}" data-price="{{ $p['price'] }}" data-name="{{ strtolower($p['name']) }}"
                ...
                <div id="noResults" class="ut-card" style="display:none;text-align:center;padding:80px 20px">
                    ...
                    <h3>No tees match</h3><p class="muted" style="margin-top:6px">Try clearing a filter or two.</p>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
<script>
    window.UT_CAT = 'All'; window.UT_SUBCAT = 'All'; window.UT_BRAND = 'All'; window.UT_SALE = false; window.UT_MAXPRICE = 120;
    function setCat(btn){
    ...
    function clearFilters(){
        ...
        document.querySelectorAll('.cat-btn:not([data-cat="All"]), .subcat-btn, .brand-btn').forEach(b => b.classList.remove('is-active'));
        document.querySelector('.brand-btn[data-brand="All"]').classList.add('is-active');
        document.getElementById('saleOnly').checked = false;
        document.querySelector('input[type="range"]').value = 120;
        document.getElementById('priceVal').textContent = '$120';
        filterProducts();
    }
    function renderActiveFilters(){
        ...
        if(window.UT_SUBCAT !== 'All') filters.push(window.UT_SUBCAT);
        if(window.UT_BRAND !== 'All') filters.push(window.UT_BRAND);
        if(window.UT_SALE) filters.push('Sale only');
        if(window.UT_MAXPRICE < 120) filters.push('Under $' + window.UT_MAXPRICE);
        container.innerHTML = '';
    ...
    function syncFilterUrl(){
        ...
        if(window.UT_MAXPRICE < 120) params.set('max_price', window.UT_MAXPRICE);
    ...
    function filterProducts(){
        ...
        document.querySelectorAll('.product-cell').forEach(function(cell){
            var ok = (window.UT_CAT==='All' || cell.dataset.cat===window.UT_CAT)
                  && (window.UT_SUBCAT==='All' || cell.dataset.subcat===window.UT_SUBCAT)
                  && (window.UT_BRAND==='All' || cell.dataset.brand===window.UT_BRAND)
                  && (!window.UT_SALE || cell.dataset.sale === '1')
                  && (+cell.dataset.price <= window.UT_MAXPRICE)
                  && (!q || cell.dataset.name.indexOf(q) > -1);
    ...
    (function restoreFilterState(){
        ...
        window.UT_MAXPRICE = +(params.get('max_price') || 120);
        document.querySelectorAll('.cat-btn, .subcat-btn, .brand-btn').forEach(function(button){
            var selected = (button.dataset.cat === window.UT_CAT && !button.dataset.subcat && window.UT_SUBCAT === 'All')
                || (button.dataset.subcat === window.UT_SUBCAT && button.dataset.cat === window.UT_CAT)
                || button.dataset.brand === window.UT_BRAND;
