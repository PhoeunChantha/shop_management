...
@section('title', __('Create Account').' — T-Shirt Shop')
...
<div class="ut-auth">
    ...
    <div class="ut-auth-form">
        ...
        <div style="width:100%;max-width:400px;margin:0 auto">
            <h1 style="font-size:clamp(28px,3vw,34px);margin-bottom:8px">{{ __('Create your account') }}</h1>
            <p class="muted" style="margin-bottom:28px;font-size:15px">{{ __('Join T-Shirt Shop and get 10% off your first order.') }}</p>
            ...
            <form class="ut-col" style="gap:16px" action="{{ route('frontend.register.store') }}" method="POST">
                ...
                <label class="ut-row" style="gap:9px;font-size:13.5px;align-items:flex-start"><input type="checkbox" required style="accent-color:var(--blue);width:16px;height:16px;margin-top:2px"> <span class="muted">{{ __('I agree to the') }} <a href="{{ route('frontend.pages.terms') }}" style="color:var(--blue);font-weight:600">{{ __('Terms') }}</a> {{ __('and') }} <a href="{{ route('frontend.pages.privacy') }}" style="color:var(--blue);font-weight:600">{{ __('Privacy Policy') }}</a>.</span></label>
                <button class="ut-btn ut-btn-ink ut-btn-block ut-btn-lg" type="submit">{{ __('Create account') }}</button>
            </form>
            <p class="muted" style="text-align:center;margin-top:26px;font-size:14px">{{ __('Already have an account?') }} <a href="{{ route('frontend.login') }}" style="color:var(--blue);font-weight:600;font-family:var(--font-head)">{{ __('Sign in') }}</a></p>
        </div>
    </div>
</div>
@endsection
...
<script>
    function pwStrength(v){
        ...
        var cols=['','#ef4444','#f97316','#eab308','#22c55e'], labels=['','{{ __('Weak') }}','{{ __('Fair') }}','{{ __('Good') }}','{{ __('Strong') }}'];
        document.querySelectorAll('.pw-bar').forEach(function(b,i){ b.style.background = i<s ? cols[s] : 'var(--border)'; });
        var lbl=document.getElementById('pwLabel'); lbl.textContent=labels[s]+' {{ __('password') }}'; lbl.style.color=cols[s]||'var(--text-2)';
    }
</script>
@endpush
