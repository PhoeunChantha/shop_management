<x-app-layout>
    ...
    <div class="admin-page admin-form-page">
        <div class="page-section-header">
            <div>
                <p class="section-kicker">{{ __('Return setup') }}</p>
                <h3>{{ __('New Return Request') }}</h3>
            </div>
        ...
        @if (! $order)
            <section class="premium-card form-panel">
                <div class="form-panel-header">
                    ...
                    <div><h3>{{ __('Select order') }}</h3><p>{{ __('Choose the order that needs a return or refund workflow.') }}</p></div>
                </div>
            ...
            <form action="{{ route('admin.returns.store') }}" method="POST" class="premium-card form-panel">
                ...
                <div class="return-form-body">
                    <section class="return-form-section">
                        <p class="section-kicker mb-3">{{ __('Reason and notes') }}</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="form-field">
                                <label for="refund_amount">{{ __('Planned refund amount') }}</label>
                            ...
                            <div class="form-field">
                                <label for="customer_note">{{ __('Customer note') }}</label>
                    ...
                    <section class="return-form-section">
                        <p class="section-kicker mb-3">{{ __('Return items') }}</p>
                        <div class="premium-table-wrap">
                            <table class="premium-table">
                                <thead><tr><th>{{ __('Return') }}</th><th>{{ __('Product') }}</th><th>{{ __('Ordered') }}</th><th>{{ __('Qty') }}</th><th>{{ __('Condition') }}</th><th class="text-end">{{ __('Line total') }}</th></tr></thead>
                                <tbody>
                                    @foreach ($order->details as $item)
                                        <tr>
                                            ...
                                            <td><input type="text" name="items[{{ $loop->index }}][condition]" value="{{ old("items.{$loop->index}.condition") }}" class="form-input" placeholder="{{ __('Unopened, damaged...') }}"></td>
                ...
                <div class="form-panel-footer">
                    <a href="{{ route('admin.returns.index') }}" class="form-cancel-button">{{ __('Cancel') }}</a>
                    <button type="submit" class="form-submit-button"><i class="fa-solid fa-check"></i>{{ __('Create Return') }}</button>
                </div>
            </form>
        @endif
    </div>
