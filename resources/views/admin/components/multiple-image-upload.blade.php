...
<div class="img-uploader"
    ...
    {{-- Dropzone --}}
    <label class="img-dropzone" :class="{ 'is-dragging': dragging }"
    ...
    <div class="img-grid">
        ...
        @foreach ($existing as $img)
            <div class="img-cell" x-show="!removed.includes({{ $img->id }})">
                ...
                <label class="img-cell__primary" title="Set as primary">
                    <input type="radio" name="{{ $primaryName }}" value="{{ $img->id }}" @checked($currentPrimary === $img->id)>
                    <i class="fa-solid fa-star"></i>
                </label>
                <button type="button" class="img-cell__remove" @click="removed.push({{ $img->id }})" title="Remove">
        ...
        <template x-for="f in files" :key="f.url">
            <div class="img-cell">
                ...
                <button type="button" class="img-cell__remove" @click="removeNew(f.i)" title="Remove">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
        </template>
