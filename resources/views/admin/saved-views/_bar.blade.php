...
<section class="saved-view-bar">
    <div class="saved-view-bar__presets">
        <span class="saved-view-bar__label"><i class="fa-solid fa-bookmark"></i> {{ __('Saved views') }}</span>
        ...
        @empty
            <span class="saved-view-empty">{{ __('No presets yet') }}</span>
        @endforelse
    </div>

    <details class="saved-view-save">
        <summary><i class="fa-solid fa-plus"></i><span>{{ __('Save current') }}</span></summary>
        <form method="POST" action="{{ route('admin.saved-views.store') }}" class="saved-view-save__form">
            ...
            <label>
                <span>{{ __('Preset name') }}</span>
                <input type="text" name="name" maxlength="80" placeholder="{{ __('e.g. Low stock') }}" required>
            </label>
            ...
            @endcan
            <button type="submit"><i class="fa-solid fa-bookmark"></i><span>{{ __('Save') }}</span></button>
        </form>
    </details>
</section>
