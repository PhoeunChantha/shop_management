...
<form action="{{ $action }}" method="POST">
    ...
    <div class="form-panel-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-field col-span-2">
            ...
            <small class="text-gray-400 dark:text-slate-500 d-block mt-1">{{ __('Shown in the storefront top bar.') }}</small>
        ...
        <div class="form-field col-span-2">
            ...
            <small class="text-gray-400 dark:text-slate-500 d-block mt-1">{{ __('Optional — makes the whole bar clickable.') }}</small>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <small class="text-gray-400 dark:text-slate-500 d-block mt-1">{{ __('Lower numbers rotate first.') }}</small>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <select name="status" id="status" class="form-input">
                <option value="1" {{ old('status', $a->status ?? 1) == 1 ? 'selected' : '' }}>{{ __('Enabled') }}</option>
                <option value="0" {{ old('status', $a->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Disabled') }}</option>
            </select>
    ...
    <div class="form-panel-footer mt-6">
        <a href="{{ route('admin.announcements.index') }}" class="form-cancel-button">{{ __('Cancel') }}</a>
