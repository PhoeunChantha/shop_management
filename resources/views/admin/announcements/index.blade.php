<x-app-layout>
    ...
    <div class="admin-page">
        <div class="page-section-header">
            <div>
                <p class="section-kicker">{{ __('Top bar messages') }}</p>
                <h3>{{ __('Announcement bar') }}</h3>
            </div>
            <a href="{{ route('admin.announcements.create') }}" class="premium-button premium-button--dark">
                <i class="fa-solid fa-plus"></i><span>{{ __('New Announcement') }}</span>
            </a>
        </div>
        ...
        <x-admin.table-card class="mt-3 orders-panel" bulk>
            ...
            <x-slot:toolbar>
                <x-table-toolbar>
                    ...
                    <x-slot:right>
                        <x-search-input name="search" placeholder="{{ __('Search messages...') }}" />
                    </x-slot:right>
                </x-table-toolbar>
            </x-slot:toolbar>
                ...
                <table class="dash-table">
                    ...
                    <tbody>
                        @forelse ($announcements as $announcement)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span class="text-gray-400 dark:text-slate-500 text-xs italic">{{ __('No link') }}</span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6">
                                    <x-admin.empty-state icon="fa-solid fa-bullhorn" title="{{ __('No announcements yet') }}"
                                        message="{{ __('Add a message for the storefront top bar.') }}" />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <x-slot:footer>
                <x-table-footer :paginator="$announcements" label="{{ __('announcements') }}" />
            </x-slot:footer>
        </x-admin.table-card>

        <x-delete-confirm-modal id="deleteAnnouncementModal" title="{{ __('Delete this announcement?') }}"
