...
<form action="{{ $action }}" method="POST">
    ...
    <div class="form-panel-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-field col-span-2">
            ...
            <input value="{{ old('question', $f->question ?? '') }}" type="text" name="question" id="question"
                class="form-input" placeholder="{{ __('e.g. How long does delivery take?') }}" required>
        ...
        <div class="form-field col-span-2">
            ...
            <textarea name="answer" id="answer" class="form-input" rows="5" required
                placeholder="{{ __('The answer shown to customers') }}">{{ old('answer', $f->answer ?? '') }}</textarea>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <input value="{{ old('category', $f->category ?? '') }}" type="text" name="category" id="category"
                class="form-input" list="faqCategories" placeholder="{{ __('e.g. Shipping') }}">
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <select name="status" id="status" class="form-input">
                <option value="1" {{ old('status', $f->status ?? 1) == 1 ? 'selected' : '' }}>{{ __('Enabled') }}</option>
                <option value="0" {{ old('status', $f->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Disabled') }}</option>
            </select>
    ...
    <div class="form-panel-footer mt-6">
        <a href="{{ route('admin.faqs.index') }}" class="form-cancel-button">{{ __('Cancel') }}</a>
