...
<form action="{{ $action }}" method="POST" x-data="{ type: '{{ $typeValue }}' }">
    ...
    <div class="form-panel-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <input value="{{ old('name', $m->name ?? '') }}" type="text" name="name" id="name"
                class="form-input" placeholder="{{ __('e.g. Standard shipping') }}" required>
        ...
        <div class="form-field col-span-2 md:col-span-1" x-show="type === 'free_over'" x-cloak>
            <label for="free_over_amount">{{ __('Free over (subtotal)') }}</label>
            <input value="{{ old('free_over_amount', $m->free_over_amount ?? '') }}" type="number" step="0.01" min="0"
                name="free_over_amount" id="free_over_amount" class="form-input" placeholder="{{ __('e.g. 50.00') }}">
            <small class="text-gray-400 dark:text-slate-500 d-block mt-1">{{ __('Orders at/above this subtotal ship free.') }}</small>
            @error('free_over_amount')<p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>@enderror
        </div>

        <div class="form-field col-span-2 md:col-span-1">
            <label for="delivery_time">{{ __('Delivery estimate') }}</label>
            <input value="{{ old('delivery_time', $m->delivery_time ?? '') }}" type="text" name="delivery_time"
                id="delivery_time" class="form-input" placeholder="{{ __('e.g. 2–4 business days') }}">
        ...
        <div class="form-field col-span-2">
            ...
            <input value="{{ old('description', $m->description ?? '') }}" type="text" name="description"
                id="description" class="form-input" placeholder="{{ __('Shown to customers at checkout') }}">
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <select name="status" id="status" class="form-input">
                <option value="1" {{ old('status', $m->status ?? 1) == 1 ? 'selected' : '' }}>{{ __('Enabled') }}</option>
                <option value="0" {{ old('status', $m->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Disabled') }}</option>
            </select>
    ...
    <div class="form-panel-footer mt-6">
        <a href="{{ route('admin.shipping.index') }}" class="form-cancel-button">{{ __('Cancel') }}</a>
