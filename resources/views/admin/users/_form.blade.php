...
<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="p-6 space-y-5">
        <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('User Name') }}</label>
        ...
        @unless ($isEdit)
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    <label for="confirm_password" class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('Confirm Password') }}</label>
        ...
        <div>
            <label for="avatar" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Avatar (Optional)') }}</label>
            <div class="avatar-upload-field">
                ...
                <div class="avatar-upload-content">
                    <div class="avatar-upload-heading">
                        <p class="avatar-upload-title">{{ __('Profile photo') }}</p>
                        <p class="avatar-upload-help">{{ __('JPEG, PNG, JPG, or GIF. Max 2MB.') }}</p>
                    </div>
                    ...
                    <p class="avatar-upload-filename" data-avatar-filename>{{ __('No file selected') }}</p>
                </div>
            </div>
            @error('avatar')
                <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
        ...
        <div>
            <label for="role" class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('Assign Role') }}</label>
    ...
    <div class="flex items-center justify-end gap-3 px-6 py-4 bg-gray-50 border-t border-gray-100">
        <a href="{{ route('admin.users.index') }}" class="text-sm font-medium text-gray-600 hover:text-gray-900 px-4 py-2.5">{{ __('Cancel') }}</a>
        <button type="submit"
