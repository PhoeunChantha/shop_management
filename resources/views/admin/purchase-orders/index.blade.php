<x-app-layout>
    ...
    <div class="admin-page">
        <div class="restock-stat-strip">
            <div class="restock-stat"><span>{{ __('Total POs') }}</span><strong>{{ number_format($stats['total']) }}</strong></div>
            <div class="restock-stat restock-stat--active"><span>{{ __('Open') }}</span><strong>{{ number_format($stats['open']) }}</strong></div>
            <div class="restock-stat"><span>{{ __('Received') }}</span><strong>{{ number_format($stats['received']) }}</strong></div>
            <div class="restock-stat"><span>{{ __('Ordered value') }}</span><strong>${{ number_format($stats['value'], 0) }}</strong></div>
        </div>

        <div class="page-section-header">
            <div>
                <p class="section-kicker">{{ __('Purchasing workflow') }}</p>
                <h3>{{ __('Supplier Restock Orders') }}</h3>
            </div>
            <a href="{{ route('admin.purchase-orders.create') }}" class="premium-button premium-button--dark">
                <i class="fa-solid fa-plus"></i><span>{{ __('New Purchase Order') }}</span>
            </a>
        </div>
        ...
        <x-admin.table-card class="restock-table-card">
            <x-slot:toolbar>
                <x-table-toolbar>
                    ...
                    <x-slot:right><x-search-input name="search" placeholder="{{ __('Search PO number or supplier...') }}" /></x-slot:right>
                </x-table-toolbar>
            </x-slot:toolbar>
            ...
            <table class="premium-table">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="7"><x-admin.empty-state icon="fa-solid fa-clipboard-list" title="{{ __('No purchase orders found') }}" message="{{ __('Create purchase orders to plan incoming inventory.') }}" /></td></tr>
                    @endforelse
                </tbody>
            </table>
