...
<form action="{{ $action }}" method="POST">
    ...
    <div class="form-panel-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <input value="{{ old('name', $r->name ?? '') }}" type="text" name="name" id="name"
                class="form-input" placeholder="{{ __('e.g. Standard VAT') }}" required>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <input value="{{ old('rate', $r->rate ?? '0.00') }}" type="number" step="0.01" min="0" max="100"
                name="rate" id="rate" class="form-input" placeholder="{{ __('e.g. 8.50') }}" required>
            @error('rate')<p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>@enderror
        </div>

        <div class="form-field col-span-2 md:col-span-1">
            <label for="is_inclusive">{{ __('Pricing') }}</label>
            <select name="is_inclusive" id="is_inclusive" class="form-input">
                <option value="0" {{ old('is_inclusive', $r->is_inclusive ?? 0) == 0 ? 'selected' : '' }}>{{ __('Exclusive (added at checkout)') }}</option>
                <option value="1" {{ old('is_inclusive', $r->is_inclusive ?? 0) == 1 ? 'selected' : '' }}>{{ __('Inclusive (price already includes tax)') }}</option>
            </select>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <input value="{{ old('country', $r->country ?? '') }}" type="text" name="country" id="country"
                class="form-input" placeholder="{{ __('Leave blank to apply everywhere') }}">
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <select name="status" id="status" class="form-input">
                <option value="1" {{ old('status', $r->status ?? 1) == 1 ? 'selected' : '' }}>{{ __('Enabled') }}</option>
                <option value="0" {{ old('status', $r->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Disabled') }}</option>
            </select>
    ...
    <div class="form-panel-footer mt-6">
        <a href="{{ route('admin.taxes.index') }}" class="form-cancel-button">{{ __('Cancel') }}</a>
