<x-app-layout>
    ...
    <div class="admin-page">
        <div class="page-section-header">
            <div>
                <p class="section-kicker">{{ __('Cart lead') }}</p>
        ...
        <div class="cart-recovery-layout">
            <x-admin.table-card class="cart-recovery-table-card">
                <x-slot:toolbar>
                    <div class="table-titlebar"><div><h3>{{ __('Cart items') }}</h3><p>{{ __('Products left behind before checkout') }}</p></div></div>
                </x-slot:toolbar>
                <table class="premium-table">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="5"><x-admin.empty-state icon="fa-solid fa-cart-shopping" title="{{ __('No cart lines saved') }}" message="{{ __('Future storefront tracking will populate item-level details.') }}" /></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </x-admin.table-card>
            ...
            <section class="premium-card cart-recovery-workflow">
                <div class="form-section__header">
                    ...
                    <div>
                        <p class="section-kicker">{{ __('Recovery status') }}</p>
                        <h3>{{ __('Workflow notes') }}</h3>
                    </div>
                </div>
                <form method="POST" action="{{ route('admin.abandoned-carts.update', $cart) }}" class="d-flex flex-column gap-3">
                    @csrf
                    @method('PATCH')
                    <x-select name="status" label="{{ __('Status') }}" :options="\App\Models\AbandonedCart::STATUSES" :value="$cart->status" required />
                    <div class="form-field">
                        <label for="admin_note">{{ __('Admin note') }}</label>
                        <textarea name="admin_note" id="admin_note" class="form-input" rows="6" placeholder="{{ __('Call notes, discount offer, recovery result...') }}">{{ old('admin_note', $cart->admin_note) }}</textarea>
                    </div>
                    <button class="premium-button premium-button--dark w-100 justify-content-center">
                        <i class="fa-solid fa-floppy-disk"></i><span>{{ __('Save workflow') }}</span>
                    </button>
                </form>
            </section>
        </div>
