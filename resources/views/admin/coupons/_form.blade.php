...
<form action="{{ $action }}" method="POST">
    ...
    <div class="form-panel-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-field col-span-2 md:col-span-1">
            <label for="code">{{ __('Coupon Code') }} <span class="text-red-500">*</span></label>
            <input value="{{ old('code', $coupon->code ?? '') }}" type="text" name="code" id="code"
                class="form-input font-mono" placeholder="{{ __('e.g. SUMMER25') }}" style="text-transform: uppercase;" required>
            <small class="text-gray-400 dark:text-slate-500 d-block mt-1">{{ __('Letters, numbers, dashes and underscores only.') }}</small>
            @error('code')
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <select name="status" id="status" class="form-input">
                <option value="1" {{ old('status', $coupon->status ?? 1) == 1 ? 'selected' : '' }}>{{ __('Enable') }}</option>
                <option value="0" {{ old('status', $coupon->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Disable') }}</option>
            </select>
            @error('status')
                <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
            @enderror
        ...
        {{-- Type --}}
        <div class="form-field col-span-2 md:col-span-1">
            <label for="type">{{ __('Discount Type') }} <span class="text-red-500">*</span></label>
        ...
        {{-- Value --}}
        <div class="form-field col-span-2 md:col-span-1">
            <label for="value">{{ __('Value') }} <span class="text-red-500">*</span></label>
            <input value="{{ old('value', $coupon->value ?? '') }}" type="number" step="0.01" min="0" name="value" id="value"
                class="form-input" placeholder="{{ __('e.g. 25') }}" required>
            <small class="text-gray-400 dark:text-slate-500 d-block mt-1">{{ __('Percent (0–100) for percentage type, or a fixed amount.') }}</small>
            @error('value')
                <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
            @enderror
        </div>
        ...
        {{-- Max discount (cap) --}}
        <div class="form-field col-span-2 md:col-span-1">
            <label for="max_discount">{{ __('Max Discount Cap') }}</label>
            <input value="{{ old('max_discount', $coupon->max_discount ?? '') }}" type="number" step="0.01" min="0" name="max_discount" id="max_discount"
                class="form-input" placeholder="{{ __('Optional — caps a percentage discount') }}">
            @error('max_discount')
                <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <input value="{{ old('usage_limit', $coupon->usage_limit ?? '') }}" type="number" min="1" name="usage_limit" id="usage_limit"
                class="form-input" placeholder="{{ __('Optional — leave blank for unlimited') }}">
            @error('usage_limit')
                <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="form-panel-footer mt-6">
        <a href="{{ route('admin.coupons.index') }}" class="form-cancel-button">{{ __('Cancel') }}</a>
