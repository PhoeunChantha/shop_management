<x-app-layout>
    ...
    <div class="admin-page">
        <div class="deal-stat-strip">
            <div class="deal-stat"><span>{{ __('Total') }}</span><strong>{{ number_format($stats['total']) }}</strong></div>
            <div class="deal-stat deal-stat--active"><span>{{ __('Active') }}</span><strong>{{ number_format($stats['active']) }}</strong></div>
            <div class="deal-stat"><span>{{ __('Scheduled') }}</span><strong>{{ number_format($stats['scheduled']) }}</strong></div>
            <div class="deal-stat"><span>{{ __('Expired') }}</span><strong>{{ number_format($stats['expired']) }}</strong></div>
        </div>

        <div class="page-section-header">
            <div>
                <p class="section-kicker">{{ __('Promotion campaigns') }}</p>
        ...
        <x-filter-card :action="route('admin.deals.index')">
            <x-select name="type" size="sm" :value="request('type')" placeholder="{{ __('All deal types') }}" :options="$types" />
            <x-select name="lifecycle" size="sm" :value="request('lifecycle')" placeholder="{{ __('Any status') }}"
                :options="['active' => 'Active now', 'scheduled' => 'Scheduled', 'expired' => 'Expired', 'disabled' => 'Disabled']" />
            <x-search-input name="search" placeholder="{{ __('Search title, badge or summary...') }}" />
        </x-filter-card>
        ...
        <x-admin.table-card bulk>
            ...
            <table class="premium-table">
                ...
                <tbody>
                    @forelse ($deals as $deal)
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span class="text-muted">{{ __('Campaign only') }}</span>
                                @endif
                            </td>
                            <td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8">
                                <x-admin.empty-state icon="fa-solid fa-tags" title="{{ __('No deals found') }}"
                                    message="{{ __('Create a flash deal, daily offer, featured campaign, or clearance sale.') }}" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <x-slot:footer>
                <x-table-footer :paginator="$deals" label="{{ __('deals') }}" />
            </x-slot:footer>
        </x-admin.table-card>

        <x-delete-confirm-modal
            id="deleteDealModal"
            title="{{ __('Delete this deal?') }}"
