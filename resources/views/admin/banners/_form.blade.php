...
<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-panel-body grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="form-field col-span-2">
            <x-image-upload name="image" label="{{ __('Banner Image *') }}" folder="banners"
        ...
        <div class="form-field col-span-2 md:col-span-1">
            <label for="kicker">{{ __('Kicker') }}</label>
            <input value="{{ old('kicker', $b->kicker ?? '') }}" type="text" name="kicker" id="kicker"
                class="form-input" placeholder="{{ __('e.g. New Season') }}">
            <small class="text-gray-400 dark:text-slate-500 d-block mt-1">{{ __('Small eyebrow text shown above the title.') }}</small>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <input value="{{ old('title', $b->title ?? '') }}" type="text" name="title" id="title"
                class="form-input" placeholder="{{ __('e.g. Summer Collection 2026') }}" required>
        ...
        <div class="form-field col-span-2 md:col-span-1">
            ...
            <select name="status" id="status" class="form-input">
                <option value="1" {{ old('status', $b->status ?? 1) == 1 ? 'selected' : '' }}>{{ __('Enabled') }}</option>
                <option value="0" {{ old('status', $b->status ?? 1) == 0 ? 'selected' : '' }}>{{ __('Disabled') }}</option>
            </select>
    ...
    <div class="form-panel-footer mt-6">
        <a href="{{ route('admin.banners.index') }}" class="form-cancel-button">{{ __('Cancel') }}</a>
